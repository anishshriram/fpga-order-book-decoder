...
**One book + physical 7-segment displays** (`sseg12.v` + `disp3.v`):
- `make flash-demo2` — two CL5641BH: **ticker name** / **best bid `DDD.D`**
- `make flash-demo3` — three: adds a **resting-order-count** display

Wire per `tangnano20k.cst`: 8 shared segment lines (220 Ω each) + N digit-selects,
each through a PNP high-side switch (1 kΩ base) to 3.3 V. Both drop the `dbg[]`
logic-analyzer taps (the extra digits reuse those pins). Renders cleanly for
tickers whose letters exist on 7-seg (AAPL yes; MSFT/NVDA/AMD have M/V that don't).
